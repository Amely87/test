...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	...
	else
	{
		...
		else
		{
			...
			if(playerSide == east && player distance cursorTarget < 4) then //Wenn Medic
			{
				if(!dialog) then
				{
					[_curTarget] call life_fnc_lacInteractionMenu;
				};
			};
		};	
	};
...
private _lakesideArray = [16019.5,16952.9,0];
private _pos = [[["Altis", _altisArray], ["Tanoa", _tanoaArray], ["Jackson_County", _lakesideArray]]] call TON_fnc_terrainSort;
...
if (isPlayer _curTarget && _curTarget isKindOf "Man") then {
    ...
    if (_isVehicle) then {
        if (!dialog) then {
            if (player distance _curObject < ((boundingBox _curObject select 1) select 0)+2 && (!(player getVariable ["ace_captives_isHandcuffed",false])) && (!(player getVariable ["playerSurrender",false])) && !life_isknocked && !life_istazed) then {
