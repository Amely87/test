...
*/
private ["_handled","_shift","_alt","_code","_ctrl","_alt","_ctrlKey","_veh","_locked","_interactionKey","_mapKey","_interruptionKeys","_player","_cop"];
...
_handled = false;
_player = player;
_cop = if(playerSide == west) then {true} else {false};
...
/*    case 48: {
	...
	//Ö-Key
	case 39: {
		if ((isNull(findDisplay 20000)) && (playerSide in ([west,independent,east]))) then {
    ...
    case _mapKey: {
        switch (playerSide) do {
			...
			case east: {if (!visibleMap) then {[] spawn life_fnc_lacMarkers;}};
        };
    };
...
/*     case 19: {
        ...
        if (_shift && playerSide isEqualTo west && {!isNull cursorObject} && {cursorObject isKindOf "Man"} && {(isPlayer cursorObject)} && {(side cursorObject in [civilian,independent])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject getVariable "Escorting")} && {!(cursorObject getVariable "ace_captives_isHandcuffed")} && {speed cursorObject < 1}) then {
    ...
    case 34: {
        ...
        if (_shift && playerSide isEqualTo civilian && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
            if ((animationState cursorObject) != "static_dead" && (currentWeapon player == primaryWeapon player || currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable ["ace_captives_isHandcuffed",false]) && !life_istazed && !life_isknocked) then {
    ...
    //Y Player Menu
    case 21: {
        if (!_alt && !_ctrlKey && !dialog && !(player getVariable ["ace_captives_isHandcuffed",false]) && {!life_action_inUse}) then {
    ...
    //F Key
    case 33: {
        if (playerSide in [west,independent] &&  {vehicle player != player} && {!life_siren_active} && {((driver vehicle player) == player)} && _ctrlKey) then {
            [] spawn {
	...
	{
		if(_shift) then {_handled = true;};
		if (_shift && !(player getVariable "ace_captives_isHandcuffed") && (vehicle player == player) && !life_knockout && !life_istazed) then
		{
			[player,"cl3_nightclubdance"] remoteExec ["life_fnc_animSync",0];
		};
	};	
	...
	case 72:
	{
		...
		if (_shift && !(player getVariable "ace_captives_isHandcuffed") && (vehicle player == player) && !life_knockout && !life_istazed) then
		{
			[player,"cl3_robotdance"] remoteExec ["life_fnc_animSync",0];
		};
	};	
	...
	case 75:
	{
		...
		if (_shift && !(player getVariable "ace_captives_isHandcuffed") && (vehicle player == player) && !life_knockout && !life_istazed) then
		{
			[player,"cl3_crazydrunkdance"] remoteExec ["life_fnc_animSync",0];
		};
	};	
	...
	case 76:
	{
		...
		if (_shift && !(player getVariable "ace_captives_isHandcuffed") && (vehicle player == player) && !life_knockout && !life_istazed) then
		{
			[player,"cl3_russiandance"] remoteExec ["life_fnc_animSync",0];
		};
	};
    ...
    case 24:
	{
		if( _ctrlKey && !_shift && !_alt ) then {
			if(vehicle player != player) then {
				if(!life_action_inUse) then {
					[] call life_fnc_coptor;
				};
			};
		};
        if( _alt && !_shift && !_ctrlKey ) then {
            _handled = true;
            if ( playerSide != civilian && (time - life_cooldown) > 2 ) then {
                life_cooldown = time;
