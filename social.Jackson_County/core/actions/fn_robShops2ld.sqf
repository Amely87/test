...
[_shop,_robber,_action,-1] remoteExec ["TON_fnc_LosDState",2];

hint "Was? Ein Alarm? Die Cops werden sicherlich gleich hier sein!!";
[2,format["LosD-City Bank wird von Rebellen ausgeraubt!"]] remoteExec ["life_fnc_broadcast",-2];
[1,format["LosD-City Bank wird von Rebellen ausgeraubt!"]] remoteExec ["life_fnc_broadcast",west];
...
if(_rip) then
{
    ...
    if!(alive _robber) exitWith {};
	[1,format["NEWS - LosD City: %1 wurde gerade ausgeraubt. Bankinhalt: $%2", _shopName, [_kassa] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",-2];
};
[_shop,_robber,_action,0] remoteExec ["TON_fnc_LosDState",2];