...
[_shop,_robber,_action,-1] remoteExec ["TON_fnc_MorrisonState",2];

hint "Was? Ein Alarm? Die Cops werden sicherlich gleich hier sein!!";
[2,format["Morrison-City Bank wird von Rebellen ausgeraubt!"]] remoteExec ["life_fnc_broadcast",-2];
[1,format["Morrison-City Bank wird von Rebellen ausgeraubt!"]] remoteExec ["life_fnc_broadcast",west];
...
if(_rip) then
{
    ...
    if!(alive _robber) exitWith {		
		...
		[1,"LakesideCity Bank: Der Täter ist verstorben!"] remoteExec ["life_fnc_broadcast",west];
		_rip = false;
		5 cutText ["","PLAIN"];
	};
	if(life_istazed) exitwith {
		...
		5 cutText ["","PLAIN"];
		[1,"LakesideCity Bank: Der Täter wurde erfolgreich gestellt!"] remoteExec ["life_fnc_broadcast",west];
    ...
    if(_robber distance _shop > 3) exitWith { 
		...
		5 cutText ["","PLAIN"];
		[1,"LakesideCity Bank: Der Täter ist ohne Geld geflüchtet!"] remoteExec ["life_fnc_broadcast",west];
		_rip = false;
		deleteMarker "Marker200";
	};
    ...
    if!(alive _robber) exitWith {};
	[1,format["NEWS - Morrison City: %1 wurde gerade ausgeraubt. Bankinhalt: $%2", _shopName, [_kassa] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",-2];
};
[_shop,_robber,_action,0] remoteExec ["TON_fnc_MorrisonState",2];